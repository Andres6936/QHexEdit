...
{
	this->showAddressArea = addressArea;
    adjust();
...
qint64 QHexEdit::cursorPosition(QPoint pos)
{
    ...
    if ((posX >= _pxPosHexX) && (posX < (_pxPosHexX + (1 + _hexCharsInLine) * _pxCharWidth)))
    {
        ...
        if (this->showAsciiArea && (posX >= _pxPosAsciiX) && (posX < (_pxPosAsciiX + (1 + _bytesPerLine) * _pxCharWidth)))
        {
...
void QHexEdit::paintEvent(QPaintEvent *event)
{
    ...
    if (event->rect() != _cursorRect)
    {
        ...
        if (this->showAddressArea)
        {
			...
			// Draw the line that separate the area of address and the area of hex view
			painter.setPen(Qt::lightGray);
			// Indicate the position in x where begin to draw the line
			const std::uint32_t xCoordinate = this->_pxPosHexX - (this->_pxGapAdrHex / 2);
			// Draw the line from top to bottom of viewport (or event)
			painter.drawLine(xCoordinate, event->rect().top(), xCoordinate, event->rect().bottom());
        }

        if (this->showAsciiArea)
        {
			// Indicate the position in x where begin to draw the line
            const std::uint32_t xCoordinate = _pxPosAsciiX - (_pxGapHexAscii / 2);
            // Draw the line that separate the area of hex view and the area of ascii view
            painter.setPen(Qt::lightGray);
			// Draw the line from top to bottom of viewport (or event)
...
void QHexEdit::resizeEvent(QResizeEvent *)
{
    if (_dynamicBytesPerLine)
    {
        ...
        setBytesPerLine(std::max(charWidth / (this->showAsciiArea ? 4 : 3),1));
    }
    adjust();
}
